    ...
    async def display_meme(self, path: str, is_meme: bool = True, duration: float = None, ready_event=None, ready_opacity: float | None = None, object_name_override: str | None = None):
        ...
        try:
            ...
            try:
                ...
                debug_print("OBSWebsockets", f"[DEBUG] Could not fetch settings for '{object_name}': {e} / {e2}")
                get_resp = None
        ...
        if default_path.exists():
            ...
            try:
                ...
                debug_print(
                    "OBSWebsockets",
                    f"Failed to restore placeholder for '{object_name}': {exc}",
                )
        else:
            debug_print(
                "OBSWebsockets",
                f"Placeholder file missing at {default_abs_path}; leaving current media in place.",
            )
